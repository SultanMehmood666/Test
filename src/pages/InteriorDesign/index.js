...
import {FetchInterrior} from "@/hooks/PagesDataRetrive";


function ShopGrid() {
  const {interriorData} = FetchInterrior();
  const dataArray = interriorData;
  ...
  useEffect(() => {
    if (interriorData) {
      setCurrentItems(interriorData);
    }
  }, [interriorData]);
  console.log("DataArray:",interriorData);
                        ...
                        {currentItems.map((product, key) => {
                          ...
                          return (
                            <Col key={key} xs={12} sm={6} lg={4}>
                              <Link href={{pathname: `Intdesign/${slug}`, query:{id: product._id}}}>
                              <RelatedProduct
                                slug={product.title}
                                baseUrl="shop/"
                                productData={product}
