...
import {FetchRealEstate} from "@/hooks/PagesDataRetrive";


function ShopGrid() {
  const {realEstateData} = FetchRealEstate();
  const dataArray = realEstateData;
  ...
  useEffect(() => {
    if (realEstateData) {
      setCurrentItems(realEstateData);
    }
  }, [realEstateData]);
                        ...
                        {currentItems.map((product, key) => {
                          ...
                          return (
                            <Col key={key} xs={12} sm={6} lg={4}>
                              <Link href={{pathname: `RealState/${slug}`, query:{id: product._id}}}>
                              <RelatedProduct
                                slug={product.title}
                                baseUrl="shop/"
                                productData={product}
